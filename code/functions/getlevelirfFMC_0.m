function [ir1,hir1,vir1,dir1] = getlevelirfFMC_0( L,LH,LV,N,horizon,Fmat,Qmat,varcoef,iamat,A0,REPS,Y0,hlast0,FF,scale,pos )
    %
    maxtrys=100;
    sigma0=A0'*A0;
     cQ=chol(Qmat);
...
% %             log(hlast0(end,:))
% hlast0
            hhat(m,:)=log(hlast0(rows(hlast0),:));
            hhats(m,:)=log(hlast0(rows(hlast0),:));
        end
        end
        end
    % 
