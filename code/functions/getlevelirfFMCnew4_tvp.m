function [ir1,ir2,ir3,ir4,hir1,hir2,hir3,hir4,...
    ...
    getlevelirfFMCnew4_tvp( L,LH,LV,N,...
    horizon,beta2v0,Qv,Qmat,beta20,Q,iamat,A0,REPS,Y0,hlast0,EX )
    %
    NN=cols(beta20);
    NNv=cols(beta2v0);
    maxtrys=100;
...
dds1=0;dds2=0;dds3=0;dds4=0;
    ...
    hhat(L:L,:)=log(hlast0);
    bhat=zeros(horizon+LL,NN);
    bhat(L:L,:)=beta20;
    bhatv=zeros(horizon+LL,NNv);
    bhatv(L:L,:)=beta2v0;
...
detxs4=detx;
    ...
    for m=LL+1:horizon+LL
          ...
          % project forward TVP paramters
    chck=-1;
        trys=1;
    
        while chck<0 && trys<maxtrys;
    bhat(m,:)=bhat(m-1,:)+randn(1,NN)*cQQ;
    ee=stability(bhat(m,:)',N,L,EX);
    if ee>0
        trys=trys+1;
    else
        chck=1;
    ...
    bhatv(m,:)=bhatv(m-1,:)+randn(1,NNv)*cQQv;
   
    varcoef=reshape(bhat(m,:),N*L+EX,N);
    Fmat=reshape(bhatv(m,:),N+(N*LV+1),N);
    FF=comp(bhat(m,:),N,L,EX);
    %%%%%%%%%%%%%%%%%%%%%
        hhatx=hhat(m-1,:);
        hhatxs1=hhats1(m-1,:);
        hhatxs2=hhats2(m-1,:);
        ...
        if m==L+1
        hhat(m,:)=[hhatx 1]*Fmat;
        hhats1(m,:)=[hhatxs1 1]*Fmat;
        hhats2(m,:)=[hhatxs2 1]*Fmat;
        hhats3(m,:)=[hhatxs3 1]*Fmat;
        hhats4(m,:)=[hhatxs4 1]*Fmat;
   ...
   %simulate data wit & without a shock
        ...
        shock=zeros(1,N);shock(1)=1;yhats1(m,:)=xhats1*varcoef+shock*A0; 
        shock=zeros(1,N);shock(2)=1;yhats2(m,:)=xhats2*varcoef+shock*A0;
        shock=zeros(1,N);shock(3)=1;yhats3(m,:)=xhats3*varcoef+shock*A0;
        shock=zeros(1,N);shock(4)=1;yhats4(m,:)=xhats4*varcoef+shock*A0;
