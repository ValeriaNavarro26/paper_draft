function [ir1,hir1,vir1,dir1] = getlevelirfFMCSimple( L,LH,LV,N,horizon,Fmat,Qmat,varcoef,iamat,A0,REPS,Y0,hlast0,FF,scale,pos )
    %
    maxtrys=100;
    sigma0=A0'*A0;
     cQ=chol(Qmat);
...
detxs=detx;
    for m=LL+1:horizon+LL
        ...
        hhatxs=hhats(m-1,:);
        for i=1:LV
          hhatx=[hhatx yhat(m-1,:)];  
          hhatxs=[hhatxs yhats(m-1,:)]; 
        end
        if m==L+1
        hhat(m,:)=[hhatx 1]*Fmat;
